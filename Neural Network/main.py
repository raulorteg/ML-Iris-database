"""
Created on Wed Aug 19 17:13:10 2020

@author: Raul Ortega Ochoa
"""

"""
    Implementation of a neural network to classify flowers (3 outputs) given
    sepal length, sepal width, petal length and petal width (4 inputs + 1)
"""
import numpy as np
import pandas
...
# initiliaze cost function history
J_history = []
...
for v in range(num_iters):
    ...
    D_1 = Delta_1/m_train + reg_lambda*Theta_1 
    D_2 = Delta_2/m_train + reg_lambda*Theta_2
    D_3 = Delta_3/m_train + reg_lambda*Theta_3
    
    # update Thetas weight with a gradient descent step
    Theta_1 = Theta_1 - alpha*D_1
    Theta_2 = Theta_2 - alpha*D_2
    Theta_3 = Theta_3 - alpha*D_3
...
print(f'finished training in {round(diff,3)} s.')
...
# stop time counter
finishTime_test = datetime.now()

diff = (finishTime_test - startTime_test).total_seconds()
print(f'finished testing in {round(diff,3)} s.')

# compute accuracy (%)
accuracy = (correct_pred/m_test)*100
print(f'Accuracy of NN on test set {round(accuracy,2)}%')
...
# stop time counter
finishTime_train = datetime.now()

diff = (finishTime_train - startTime_train).total_seconds()
#print(f'finished test on training set in {round(diff,3)} s.')

# compute accuracy (%)
accuracy = (correct_pred/m_train)*100
print(f'Accuracy of NN on training set {round(accuracy,2)}%')
